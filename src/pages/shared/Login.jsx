...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.user.mustChangePassword && data.user.role === "cliente") {
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-white/25 backdrop-blur-xl border border-white/40 rounded-3xl p-8 shadow-2xl ring-1 ring-white/20">
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {error && (
                <div className="bg-red-500/20 backdrop-blur-sm border border-red-300/40 rounded-xl p-3 shadow-lg">
                  <Text size="1" className="text-red-200 text-center block">
                    {error}
                  </Text>
                </div>
              )}
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    Accedi <ArrowRight className="inline-block ml-1 w-4 h-4" />
                  </>
                )}
              </button>
            </form>
